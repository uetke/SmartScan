...
class adq():
    """ Class for debugging the program. 
    Same input and outputs than the real class, but with no interaction with any device. 
    """
    def __init__(self):
        DEVICENUMBER = 1 # By default this is the number
        RAISE_EXCEPTIONS = 1
        ...
        self.running = False
        self.logger = logging.getLogger(get_all_caller())
        self.logger.info('Init the adq class')
    ...
    def wait(self,process=9,ref_time=0.1):
        ...
        self.logger = logging.getLogger(get_all_caller())
        self.logger.info("Waiting")
        while int(1.2*np.random.rand(1)):
            time.sleep(ref_time)  
    ...
    def get_fast_timetrace(self,detect,duration=1,acc=.1):
        ...
        self.start(process=8)
        time.sleep(duration)
        array = np.random.rand(num_ticks)
        return array       
    ...
    def get_QPD(self,duration=1,acc=.0005):
        ...
        #self.adw.Set_Processdelay(self.proc_num,delay)
        self.start(8)
        while bool(int(1.4*np.random.rand(1))):
            time.sleep(0.1)
        arrayX = np.array(list(self.get_data(176,num_ticks)))
        arrayY = np.array(list(self.get_data(175,num_ticks)))
        arrayZ = np.array(list(self.get_data(174,num_ticks)))
    ...
    def get_timetrace_dynamic(self,detect,duration=1,acc=0.01):
        ...
        if not self.running:
            ...
            self.set_par(par.properties['Case'],3)
            self.start(9)
            self.running = int(2*np.random.rand(1))
            self.array = np.array(list(self.get_fifo(fifo.properties['Scan_data'])))
            split_data = []
            for i in range(len(detect)):
                length = np.floor(len(self.array)/len(detect))*len(detect)
